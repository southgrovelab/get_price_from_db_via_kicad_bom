...
import sys
from modules.file_select import select_file
...
component_table = "tbl_stock_minimal_components"
...
# Function moved to modules/select_file.py for test

#**********************************************************************

def process_sql(selected):
    print(f"**************************** - Selected file {selected} file - ****************************")
...
# SQL-Query
    for mouser_no, quantity in zip(column_mouser, column_qty):
        if pd.isna(mouser_no):
            mouser_no = None
        sql_query = f"SELECT box, price FROM {component_table} WHERE supplier_part_number= %s"
        data = (mouser_no,)
...
# Added this to print to pdf
    ...
    # Create PDF
    pdf = SimpleDocTemplate(f"{file_path_updated}{edited}{kicad_project_name}.pdf", pagesize=landscape(A4))
...
# Add the text paragraph
    ...
    info_text = Paragraph("Additional costs may apply such as pcb, box, other components and hardware.", info_style)
#    story.append(title_text)
